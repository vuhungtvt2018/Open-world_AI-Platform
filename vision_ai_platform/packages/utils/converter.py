...
async def convert_ndjson_to_yolo(ndjson_path: str | Path, output_path: str | Path | None = None) -> Path:
    ...
    if is_classification:
        ...
        classify_path = dataset_dir / ".ndjson.yaml"
        with open(str(classify_path), "w", encoding="utf-8") as f:
            yaml.safe_dump({"names": classification_names}, f, sort_keys=False, allow_unicode=True, width=-1)
        return dataset_dir
    else:
        ...
        with open(str(yaml_path), "w", encoding="utf-8") as f:
            yaml.safe_dump(data_yaml, f, sort_keys=False, allow_unicode=True, width=-1)
        return yaml_path
