...
def torch_load(*args, **kwargs):
    ...
    """
    from vision_ai_platform.packages.utils.device_utils import TORCH_1_13
...
def onnx_export_patch():
    ...
    from vision_ai_platform.packages.utils.device_utils import TORCH_2_9
...
@contextmanager
def override_configs(cfg, overrides: dict[str, Any] | None = None):
    """Context manager to temporarily override configurations in args.

    Args:
        cfg (YOLOConfig): Original configuration arguments.
        overrides (dict[str, Any] | None): Dictionary of overrides to apply.

    Yields:
        (YOLOConfig): Configuration arguments with overrides applied.
    """
    if overrides:
        original_args = copy(cfg)
        for key, value in overrides.items():
            setattr(cfg, key, value)
        try:
            yield cfg
        finally:
            cfg.__dict__.update(original_args.__dict__)
    else:
        yield cfg
