...
class RTDETRPredictor(Predictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        ...
        The method filters detections based on confidence and class if specified in `self.cfg`. It converts model
        ...
        for bbox, score, label, orig_img, img_path in zip(bboxes, scores, labels, orig_imgs, self.batch[0]):
            ...
            idx = score.squeeze(-1) > self.cfg.conf
            if self.cfg.classes is not None:
                idx = (label == torch.tensor(self.cfg.classes, device=label.device)).any(1) & idx
            pred = torch.cat([bbox, score, label], dim=-1)[idx][: self.cfg.max_det]
