...
from vision_ai_platform.packages.ai.data.utils import check_det_dataset, convert_ndjson_to_yolo_if_needed
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if trainer is None:  # standalone val; training sets classes via on_pretrain_routine_end callback
            ...
            if not isinstance(model, torch.nn.Module):
                ...
                model = load_checkpoint(model or self.cfg.model, device=self.device)[0]
            model.eval().to(self.device)
            self.cfg.data = convert_ndjson_to_yolo_if_needed(self.cfg.data)  # match BaseValidator dataset handling
            names = [name.split("/", 1)[0] for name in check_det_dataset(self.cfg.data)["names"].values()]
