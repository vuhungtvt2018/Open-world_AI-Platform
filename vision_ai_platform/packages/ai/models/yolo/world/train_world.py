...
from vision_ai_platform.packages.utils import DATASETS_DIR, DEFAULT_CFG, LOGGER
...
class WorldTrainerFromScratch(WorldTrainer):
    ...
    def build_dataset(self, img_path, mode="train", batch=None):
        ...
        if mode != "train":
            return build_yolo_dataset(self.cfg, img_path, batch, self.data, mode=mode, rect=False, stride=gs)
        datasets = [
            build_yolo_dataset(self.cfg, im_path, batch, self.training_data[im_path], stride=gs, multi_modal=True)
    ...
    def get_dataset(self):
        ...
        """
        final_data = {}
        self.cfg.data = data_yaml = self.check_data_config(self.cfg.data)
        ...
        if self.cfg.single_cls:  # consistent with base trainer
    ...
    def final_eval(self):
        ...
        """
        val = self.cfg.data["val"]["yolo_data"][0]
