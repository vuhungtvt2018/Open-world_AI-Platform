...
from vision_ai_platform.packages.ai.data import YOLOConcatDataset, build_dataloader, build_yolo_dataset
...
from vision_ai_platform.packages.ai.models.yolo.segment import SegmentationValidator
from vision_ai_platform.packages.ai.nn.modules.head import YOLOEDetect
from vision_ai_platform.packages.ai.nn.tasks import YOLOEModel
from vision_ai_platform.packages.utils import LOGGER
from vision_ai_platform.packages.utils.device_utils import select_device, smart_inference_mode
...
class YOLOEDetectValidator(DetectionValidator):
    ...
    def get_vpe_dataloader(self, data: dict[str, Any]) -> torch.utils.data.DataLoader:
        ...
        dataset = build_yolo_dataset(
            self.cfg,
            data.get(self.cfg.split, data.get("val")),
            self.cfg.batch,
            data,
            mode="val",
            rect=False,
    ...
    def __call__(
        ...
        if trainer is not None:
            ...
            self.device = select_device(self.cfg.device, verbose=False)
            ...
            data = check_det_dataset(refer_data or self.cfg.data)
            names = [name.split("/", 1)[0] for name in list(data["names"].values())]

            if refer_data is not None:
                eval_data = check_det_dataset(self.cfg.data)
