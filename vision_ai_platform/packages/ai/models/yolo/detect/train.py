...
class DetectionTrainer(Trainer):
    ...
    def get_dataloader(self, dataset_path: str, batch_size: int = 16, rank: int = 0, mode: str = "train"):
        ...
        return build_dataloader(
            ...
            shuffle=shuffle,
            rank=rank,
            drop_last=self.cfg.compile and mode == "train",
        )
    ...
    def preprocess_batch(self, batch: dict) -> dict:
        ...
        if self.cfg.multi_scale > 0.0:
            ...
            sz = (
                random.randrange(
                    max(self.stride, int(self.cfg.imgsz * (1.0 - self.cfg.multi_scale))),  # min imgsz
                    int(self.cfg.imgsz * (1.0 + self.cfg.multi_scale) + self.stride),  # max imgsz
                )
                // self.stride
                * self.stride
            )  # size
    ...
    def set_model_attributes(self):
        ...
        # self.cfg.box *= 3 / nl  # scale to layers
        # self.cfg.cls *= self.data["nc"] / 80 * 3 / nl  # scale to classes and layers
        # self.cfg.cls *= (self.cfg.imgsz / 640) ** 2 * 3 / nl  # scale to image size and layers
        self.model.nc = self.data["nc"]  # attach number of classes to model
        self.model.names = self.data["names"]  # attach class names to model
        self.model.args = self.cfg  # attach hyperparameters to model
        if getattr(self.model, "end2end", False):
            self.model.set_head_attr(max_det=self.cfg.max_det)

    def set_model_names_for_load(self, model):
        """Set target dataset names before loading weights so cls heads can remap by name."""
        if getattr(self.cfg, "cls_remap", True) and self.data.get("names"):
    ...
    def compute_class_weights(self, class_counts):
        ...
        return (1.0 / class_counts) ** self.cfg.cls_pw  # apply power directly
    ...
    def set_class_weights(self):
        ...
        """
        assert 0 <= self.cfg.cls_pw <= 1.0, "cls_pw must be in the range [0, 1]"
        if self.cfg.cls_pw == 0.0:
            return
    ...
    def auto_batch(self):
        ...
        """
        cache_orig = self.cfg.cache
        self.cfg.cache = False
        train_dataset = self.build_dataset(self.data["train"], mode="train", batch=16)
        self.cfg.cache = cache_orig
