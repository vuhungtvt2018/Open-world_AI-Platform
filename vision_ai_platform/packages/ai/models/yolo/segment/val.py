...
class SegmentationValidator(DetectionValidator):
    ...
    def init_metrics(self, model: torch.nn.Module) -> None:
        ...
        self.process = ops.process_mask_native if self.cfg.save_json or self.cfg.save_txt else ops.process_mask
    ...
    def plot_predictions(self, batch: dict[str, Any], preds: list[dict[str, torch.Tensor]], ni: int) -> None:
        ...
        for p in preds:
            masks = p["masks"]
            if masks.shape[0] > self.cfg.max_det:
                LOGGER.warning(f"Limiting validation plots to 'max_det={self.cfg.max_det}' items.")
            p["masks"] = torch.as_tensor(masks[: self.cfg.max_det], dtype=torch.uint8).cpu()
        super().plot_predictions(batch, preds, ni, max_det=self.cfg.max_det)  # plot bboxes
