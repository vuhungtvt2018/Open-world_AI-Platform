...
class ClassificationValidator(Validator):
    ...
    def preprocess(self, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        batch["img"] = batch["img"].half() if self.cfg.quantize == 16 else batch["img"].float()
    ...
    def build_dataset(self, img_path: str) -> ClassificationDataset:
        ...
        return ClassificationDataset(root=img_path, args=self.cfg, augment=False, prefix=self.cfg.split)
    ...
    def get_dataloader(self, dataset_path: Path | str, batch_size: int) -> torch.utils.data.DataLoader:
        ...
        return build_dataloader(dataset, batch_size, self.cfg.workers, rank=-1)
