...
class ClassificationTrainer(Trainer):
    ...
    def get_model(self, cfg=None, weights=None, verbose: bool = True):
        ...
        for m in model.modules():
            if self.cfg.pretrained is False and hasattr(m, "reset_parameters"):
                m.reset_parameters()
    ...
    def setup_model(self):
        ...
        if str(self.model) in torchvision.models.__dict__:
            self.model = torchvision.models.__dict__[self.model](
                weights="IMAGENET1K_V1" if self.cfg.pretrained else None
            )
            ckpt = None
        else:
    ...
    def build_dataset(self, img_path: str, mode: str = "train", batch=None):
        ...
        """
        return ClassificationDataset(root=img_path, args=self.cfg, augment=mode == "train", prefix=mode)
    ...
    def get_dataloader(self, dataset_path: str, batch_size: int = 16, rank: int = 0, mode: str = "train"):
        ...
        loader = build_dataloader(dataset, batch_size, self.cfg.workers, rank=rank, drop_last=self.cfg.compile)
