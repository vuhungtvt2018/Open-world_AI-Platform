...
from vision_ai_platform.packages.ai.data.utils import FORMATS_HELP_MSG, IMG_FORMATS, VID_FORMATS
from vision_ai_platform.packages.utils import IS_COLAB, IS_KAGGLE, LOGGER, ops
    ...
    def __next__(self) -> tuple[list[str], list[np.ndarray], list[str]]:
        ...
        while len(imgs) < self.bs:
            ...
            if self.video_flag[self.count]:
                ...
                # Handle image files
                self.mode = "image"
                im0 = cv2.imread(path, flags=self.cv2_flag)  # BGR
                if im0 is None:
