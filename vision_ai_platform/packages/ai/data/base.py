...
from vision_ai_platform.packages.ai.data.utils import FORMATS_HELP_MSG, HELP_URL, IMG_FORMATS, check_file_speeds
from vision_ai_platform.packages.core.config import BaseConfig
from vision_ai_platform.packages.utils import LOCAL_RANK, LOGGER, NUM_THREADS
...
class BaseDataset(Dataset):
            ...
            if fn.exists():  # load npy
                try:
                    ...
                    if npy_channels != self.channels:
                        ...
                        im = cv2.imread(f, flags=self.cv2_flag)
                except Exception as e:
    ...
    def cache_images_to_disk(self, i: int) -> None:
        ...
        if not f.exists():
            try:
                np.save(f.as_posix(), cv2.imread(self.im_files[i], flags=self.cv2_flag), allow_pickle=False)
            except Exception as e:
