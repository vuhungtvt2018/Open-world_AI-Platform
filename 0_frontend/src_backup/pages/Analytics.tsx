...
export default function Analytics({ mode: globalMode }: AnalyticsProps) {
  const [localMode, setLocalMode] = React.useState<AnalyticsMode>('counting');
  const mode: AnalyticsMode = globalMode
    ? (globalMode === 'COUNTING' ? 'counting' : 'inspection')
    : localMode;
        ...
        <div className="header-actions">
          ...
          {!globalMode && (
            <div className="analytics-mode-switch">
              <button className={mode === 'counting' ? 'active' : ''} onClick={() => setLocalMode('counting')}>Counting</button>
              <button className={mode === 'inspection' ? 'active' : ''} onClick={() => setLocalMode('inspection')}>Inspection</button>
            </div>
          )}
          <div
