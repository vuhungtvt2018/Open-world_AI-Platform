...
interface GlobalMonitoringProps {
  mode?: SystemMode;
}

export default function GlobalMonitoring({ mode: globalMode = 'INSPECTION' }: GlobalMonitoringProps) {
  const mode = globalMode === 'COUNTING' ? 'counting' : 'inspection';
  ...
  return (
    <div className="dashboard-container">
      <header className="dashboard-header">
        <div>
          <h1 className="monitoring-title">Global Monitoring</h1>
          <p className="text-muted text-sm mt-1">Factory overview and edge node status ({globalMode} Mode)</p>
        </div>

        <div className="header-actions">
          <div className="status-badge glass-panel glow-success">
            <CheckCircle2 size={16} color="var(--success)" />
            <span>AI Inference Node Online ({globalMode})</span>
          </div>
        </div>
      </header>
